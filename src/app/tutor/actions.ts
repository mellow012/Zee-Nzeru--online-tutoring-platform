...
export async function getTutorDashboardData(): Promise<TutorDashboardData> {
  ...
  const [
    ...
    supabase
      .from('tutor_profiles')
      .select('rating, total_students, verification_status, rejection_reason')
      .eq('user_id', tutorId)
      .single(),
  ]);
  ...
  return {
    ...
    pendingRequests,
    upcomingByDay,
    verificationStatus: tutorProfile?.verification_status ?? null,
    rejectionReason: tutorProfile?.rejection_reason ?? null
  };
}
