...
export function TutorDashboard({ data }: { data: TutorDashboardData }) {
  ...
  const { stats, todaySessions, pendingRequests, upcomingByDay, verificationStatus, rejectionReason } = data;
  ...
  return (
    <div className="min-h-screen bg-gray-50/50">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 space-y-8">
        ...
        {/* Profile Verification Attention Banners */}
        {verificationStatus === 'pending_review' && (
          <div className="p-4 rounded-xl border border-blue-100 bg-blue-50/40 text-blue-800 flex items-start gap-3 shadow-sm animate-in fade-in duration-300">
            <Clock className="w-5 h-5 text-blue-500 shrink-0 mt-0.5" />
            <div>
              <p className="text-sm font-semibold">Application Under Review ⏳</p>
              <p className="text-xs text-blue-700/95 mt-0.5">
                Our administrative team is currently reviewing your application credentials. You will receive a real-time notification as soon as it is approved, enabling you to accept bookings and start teaching!
              </p>
            </div>
          </div>
        )}
        ...
        {verificationStatus === 'rejected' && (
          <div className="p-4 rounded-xl border border-red-200 bg-red-50/60 text-red-800 flex items-start gap-3 shadow-sm animate-in shake duration-500">
            <AlertCircle className="w-5 h-5 text-red-600 shrink-0 mt-0.5" />
            <div className="flex-1">
              <p className="text-sm font-semibold">Verification Attention Required ⚠️</p>
              <p className="text-xs text-red-700 mt-0.5">
                Your application could not be verified in its current state.
              </p>
              {rejectionReason && (
                <div className="mt-2 p-2.5 rounded-lg bg-red-100/50 border border-red-200/40 text-xs font-mono text-red-900">
                  <span className="font-semibold block mb-0.5">Administrator Feedback:</span>
                  {rejectionReason}
                </div>
              )}
              <Button
                size="sm"
                variant="outline"
                className="mt-3 border-red-200 text-red-700 hover:bg-red-100/50 text-xs h-7 gap-1"
        ...
        {verificationStatus === 'not_submitted' && (
          <div className="p-4 rounded-xl border border-amber-200 bg-amber-50/50 text-amber-800 flex items-start gap-3 shadow-sm animate-in fade-in duration-300">
            <AlertCircle className="w-5 h-5 text-amber-500 shrink-0 mt-0.5" />
            <div>
              <p className="text-sm font-semibold">Complete Profile Submission 📝</p>
              <p className="text-xs text-amber-700 mt-0.5">
                Your tutor profile has not been submitted for official verification review yet. Please complete your subject offerings, educational credentials, and availability to submit for admin approval.
              </p>
              <Button
                size="sm"
