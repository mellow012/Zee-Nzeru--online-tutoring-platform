...
export async function getAdminActivitySessions(): Promise<AdminActivitySession[]> {
  const { supabase } = await requireAdmin();

  const { data: sessions, error } = await supabase
    .from('sessions')
    .select('id, student_id, tutor_id, subject, scheduled_start_time, scheduled_end_time, actual_start_time, actual_end_time, status, price, currency')
