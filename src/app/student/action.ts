...
export interface TutorCard {
  ...
  languages: string[];
  latestReview?: { rating: number; comment: string | null; reviewer_name: string } | null;
}
...
export async function getStudentDashboardData(): Promise<StudentDashboardData> {
  ...
  // Deduplicate tutors by user_id in case of bad database state
  const uniqueTutors = Array.from(new Map((tutorProfileRows ?? []).map(t => [t.user_id, t])).values());

  const suggestedTutors: TutorCard[] = uniqueTutors.map((tp) =>
...
export async function searchTutors(
  ...
  // ── Step 4: Fetch latest public review for each tutor ──────────────────────────
  ...
  for (const r of reviews ?? []) {
    if (!latestReviewMap.has(r.reviewee_id)) {
      const reviewerProfile = Array.isArray(r.reviewer) ? r.reviewer[0] : r.reviewer;
      latestReviewMap.set(r.reviewee_id, {
        rating: r.rating,
        comment: r.comment,
        reviewer_name: (reviewerProfile as any)?.full_name || 'Anonymous Student'
      });
    }
  }

  return {
    tutors: tutorProfileRows.map((tp) => toTutorCard(tp as any, profileMap, latestReviewMap)),
    total:  count ?? 0,
  };
}
